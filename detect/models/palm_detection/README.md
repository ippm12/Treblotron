...
to drive the "removing darts" state in `detect/dart_detector.cpp`
...
the constants in `detect/dart_detector.cpp` (`PALM_INPUT_NAME`,
...
- `input`: `float32 [1, 192, 192, 3]` (**NHWC**, RGB, range **`[0, 1]`**)
  ...
  - **Empirically verified**: feeding `[-1, 1]` to the *full* BlazePalm
    ...
    See the palm-preprocessing comment in `detect/dart_detector.cpp` for
...
Tensor names hardcoded in `detect/dart_detector.cpp` —
