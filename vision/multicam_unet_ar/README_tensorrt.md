...
3. Append a 720x720 mask channel containing the conditioning state:
   the sum of gaussian blobs (sigma=5.0, in input pixels) at every
   dart tip the model has already placed; all-zero when no darts are
   pre-placed.  Pass as the 10th channel.
4. Layout is `NCHW`.

## Outputs
- `heatmap`        — `(1, 360, 360)` pre-sigmoid logits.  Apply sigmoid.
- `offset`         — `(1, 2, 360, 360)` sub-pixel refinement in cell units.
- `exist_logit`    — `(1,)` pre-sigmoid.  Gate peak extraction on >= 0.
  Always produce at most one peak per forward (argmax on the heatmap,
  threshold 0.55 on sigmoid, refined by offset at that cell).
